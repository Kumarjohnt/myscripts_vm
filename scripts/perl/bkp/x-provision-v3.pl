...
my %opts = ( 
	...
	customization => {type => "=s", help => "Optional & clone only: ip:x.x.x.x,mask:x.x.x.x,gw:x.x.x.x,hostname:xxxx. dns and domain are optional. use single quote for multiple values, such as dns:'10.188.8.1 10.188.8.2'. equal sign can be used in lieu of colon. 'hostname' overwrites 'newname'", required =>0},
);
...
if ($customization) {
	...
	if ($op =~ $MOVE) { print color('red'). "Customization does not apply to $op operation thus will be skipped\n".color('reset'); } 
		...
		printf "%10s: %-20s\n", $_, $custParams{$_} for (keys %custParams);
		
		# customization hostname overwrite clone target name;
		# also cut it short as you don't want to see fqdn as vm name
		$newvm = $custParams{hostname} if $custParams{hostname};
		$newvm =~ s/\..*//g;
		$cloneto = "as $newvm";
	}
	...
	unless ($custParams{dns}) {
		printf "%10s: %-20s\n", 'dns', "['10.188.8.1', '10.188.8.2']" . ' (default)';
...
$host_view = $dstVIM->find_entity_view(view_type => 'HostSystem', filter=>{'name' => $host}, 
	properties => ['name','parent','datastore','runtime.inMaintenanceMode','network']);
...
my $cloSpec = VirtualMachineCloneSpec->new( config => $vmconfig, customization => $customizationSpec, location => $reloSpec, template => 0, powerOn => 0, @snap);
