...
if ($customization) {
	...
	# remove the dot before and after the domain name, as it appears people understand the term "domain name" differently
	if ($custParams{domain}) { $custParams{domain} =~ s/^\.?(.*?)\.?$/$1/; }
	...
	if ($custParams{hostname} && $custParams{hostname} =~ /\./) { $custParams{hostname} =~ s/\..*//g; }

	#####
	if ($op =~ $MOVE) { print color('red'). "Customization does not apply to $op operation thus will be skipped\n".color('reset');} 
	else {
		print color('blue'). "##### Customizing IP configurations:\n".color('reset');
		for (keys %custParams) { printf "%10s: %-20s\n", $_, $custParams{$_};}
...
if ($hardware) {
	...
	if ($op =~ $MOVE) { print color('red'). "Customization does not apply to $op operation thus will be skipped\n".color('reset');} 
		...
		my ($memoryMB, $numCoresPerSocket,$numSockets);
		print color('blue'). "##### Customizing hardware resource allocations\n".color('reset');
		...
		# get cpu. Cpu can be specified with as x*y, or simply x. If x*y, then $numSockets = x and $numCoresPerSocket = y.
		...
		if (defined $hwParams{cpu}) {
			...
			# we need to figure our NUMA topology in this case
	       		if ($hwParams{cpu} =~ /^\d+\z/) {
				$numSockets = int($hwParams{cpu}/$hostCoresPerSocket) + 1;
				$numSockets = $hostSockets if ($numSockets > $hostSockets);
				$numCoresPerSocket = int($hwParams{cpu}/$numSockets);
				$numCoresPerSocket += 1 if ($hwParams{cpu}%$numSockets); 
			}

			# we'll take as is since we have socket and core. Just don't go overboard
			elsif ($hwParams{cpu} =~ /^\d+x\d+\z/) { ($numSockets, $numCoresPerSocket) = split /x/, $hwParams{cpu}; }
			if ($numSockets && $numCoresPerSocket) {
				printf "%10s %-20s\n", "CPU:", $numSockets*$numCoresPerSocket . "(${numSockets}x${numCoresPerSocket})";
				print color('red'). "Alert: you are asking for more CPU sockets than host has!\n".color('reset') if ($numSockets>$hostSockets);
				push (@hwFilter, (numCPUs => $numSockets*$numCoresPerSocket, numCoresPerSocket=> $numCoresPerSocket));
			}
		}
	}
}
...
if (@pgs && @nics) {
	foreach my $nic (@nics) { 
		...
		else { &cleanUp("Target network $pgs[$num] for ". $nic->deviceInfo->label =~ s/Network adapter /NIC #/r.": not found!");}
		$num++;
	}
}
