###############################################################################
## Script to failover NS1 cdnorigin record for my.vmware.com                 ##
## Maintainer : Neha G							     ##
...
_URL = {
	"url_switch" : "https://api.nsone.net/v1/zones/cdnorigin.vmware.com/myvmwareincap.cdnorigin.vmware.com/A"
}

_PAYLOAD_SC2 = json.dumps({ "zone" : "cdnorigin.vmware.com" ,
                            "domain": "myvmwareincap.cdnorigin.vmware.com",
                            "type": "A",
                            "answers":[{"answer": ["208.91.0.1"]}] })

_PAYLOAD_WDC = json.dumps({ "zone" : "cdnorigin.vmware.com" ,
                            "domain":"myvmwareincap.cdnorigin.vmware.com",
                            "type": "A",
                            "answers":[{"answer": ["208.91.2.247"]}] })
...
def dig():
        time.sleep(2)
        cmd='dig @dns1.p05.nsone.net myvmwareincap.cdnorigin.vmware.com +short'
        proc=subprocess.Popen(shlex.split(cmd),stdout=subprocess.PIPE)
        out,err=proc.communicate()
        print("my.vmware.com origin record is resolving to -> " + out)

def TD_Runner():
    args = TD_Argument_Parser().parse_args()
    if args.action == 'status':
        dc = TD_GetStatusFromCDN('https://107.154.106.19/web/csp/','my.vmware.com')
        if dc is not None:
