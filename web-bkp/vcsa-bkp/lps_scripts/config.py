...
class configurations:
    def __init__(self):
        self.api_url ='https://linux-repo.vmware.com/api/v1/'
        self.script_version = '2.8'
        self.dmz = 'Trust'
        self.authtype = 'ldap'
...
## Defining URL Functions
    ...
    def hostname_uuid(self,hn):
        uuidurl = 'http://priyank-test-c7.vmware.com/api/v1/cilt/get-vm-uuid/' + hn + '/'
        uuid_cmd = 'curl ' + '-k'  + ' -w' + ' "\n\nHTTPStatusCode: %{http_code}\n" ' + '--location --request GET ' + uuidurl + ' --header "Content-Type: application/json"'
        cmd_exe = subprocess.Popen(uuid_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out =((cmd_exe.stdout.read()).split('\n')[0])
